...
def isMultDivOk(s, result):
    ...
    if re.search("/", s):
        ...
        if int(divs[1]) == 0:
            ...
            if isResultOk(s, result) and ((int(divs[0]) % int(divs[1])) == 0) and eval(
                    s) > 0:  # 除法，除数不能为0，并且结果在范围内,并且整除无余数
                return True
            else:
                return False
    if re.search("\*", s):
...
def getRandomSymbols(symbols, step):
    ...
    for i in range(step):
        ...
        if 0 in sym:
            sym.remove(0)
        index = random.randint(0, len(sym) - 1)
        new_list.append(symbols[i][index])
    return new_list
...
def getRandomNum(f, step):
    ...
    for i in range(0, step + 1):
        new_list.append(random.randint(f[i][0], f[i][1]))
